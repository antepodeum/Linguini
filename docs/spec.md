...
The macro reads CLDR data from `crates/linguini-cldr-macros/vendor/cldr-json`.
That vendor checkout is local-only and is not included in the published crate.
When building from a published crate, including `cargo install linguini-cli`, the
macro fetches the pinned upstream ref with `git fetch --depth=1` using
`crates/linguini-cldr-macros/cldr-json.toml` if the checkout is missing. Set
`LINGUINI_CLDR_AUTO_FETCH=0` to force an explicit source-preparation step and fail
with an actionable missing-checkout error instead.

The long-term release target is stricter: published crates should ship generated
Rust CLDR tables and should not fetch network data during normal Cargo builds.
