...
function pluralFr(value: number | bigint | string): string {
  if (value === "zero" || value === "one" || value === "two" || value === "few" || value === "many" || value === "other") return value;
  const operands = pluralOperands(value);
  if ((pluralOperandMatches(operands.i, undefined, false, [[0n, 0n], [1n, 1n]]))) return "one";
...
function formatDate(
  ...
  switch (options.style ?? "medium") {
    case "full":
      return ["dimanche", "lundi", "mardi", "mercredi", "jeudi", "vendredi", "samedi"][date.getUTCDay()] + " " + String(date.getUTCDate()) + " " + ["janvier", "février", "mars", "avril", "mai", "juin", "juillet", "août", "septembre", "octobre", "novembre", "décembre"][date.getUTCMonth()] + " " + String(date.getUTCFullYear());
    case "long":
      return String(date.getUTCDate()) + " " + ["janvier", "février", "mars", "avril", "mai", "juin", "juillet", "août", "septembre", "octobre", "novembre", "décembre"][date.getUTCMonth()] + " " + String(date.getUTCFullYear());
    case "short":
      return padNumber(date.getUTCDate(), 2) + "/" + padNumber(date.getUTCMonth() + 1, 2) + "/" + String(date.getUTCFullYear());
    default:
      return String(date.getUTCDate()) + " " + ["janv.", "févr.", "mars", "avr.", "mai", "juin", "juil.", "août", "sept.", "oct.", "nov.", "déc."][date.getUTCMonth()] + " " + String(date.getUTCFullYear());
  }
}
...
const __lgl_form_6D61696E2E4672756974 = {
  apple: { Gender: "neuter", emoji: "🍎", nom: (value: number | bigint | string) => selectBranch(pluralFr(value), { one: "pomme", _: "pommes" }) },
  pear: { Gender: "female", emoji: "🍐", nom: (value: number | bigint | string) => selectBranch(pluralFr(value), { one: "poire", _: "poires" }) },
  orange: { Gender: "male", emoji: "🍊", nom: (value: number | bigint | string) => selectBranch(pluralFr(value), { one: "orange", _: "oranges" }) },
} as const;

function __lgl_name_6D61696E2E53697A65576F7264(__lgl_p0: __lgl_name_6D61696E2E53697A65): string {
  return selectBranch(String(__lgl_p0), { small: (): string => "petit", big: (): string => "grand" })();
}
...
function __lgl_name_6D61696E2E53697A6541646A(__lgl_p0: __lgl_name_6D61696E2E53697A65, __lgl_p1: number | bigint | string, __lgl_p2: __lgl_name_6D61696E2E47656E646572): string {
  return selectBranch(String(__lgl_p0), { small: (): string => selectBranch(pluralFr(__lgl_p1), { one: (): string => selectBranch(String(__lgl_p2), { male: (): string => "petit", female: (): string => "petite", neuter: (): string => "petit", _: (): string => "petit" })(), _: (): string => selectBranch(String(__lgl_p2), { male: (): string => "petits", female: (): string => "petites", neuter: (): string => "petits", _: (): string => "petits" })() })(), big: (): string => selectBranch(pluralFr(__lgl_p1), { one: (): string => selectBranch(String(__lgl_p2), { male: (): string => "grand", female: (): string => "grande", neuter: (): string => "grand", _: (): string => "grand" })(), _: (): string => selectBranch(String(__lgl_p2), { male: (): string => "grands", female: (): string => "grandes", neuter: (): string => "grands", _: (): string => "grands" })() })(), _: (): string => "normaux" })();
}
...
function __lgl_name_6D61696E2E44656C697665726564(__lgl_p0: number | bigint | string, __lgl_p1: __lgl_name_6D61696E2E47656E646572): string {
  return selectBranch(pluralFr(__lgl_p0), { one: (): string => selectBranch(String(__lgl_p1), { _: (): string => "Livré" })(), _: (): string => "Livrés" })();
}

export const main = {
  codegen: {
    kicker: "Génération de code",
    title: "Un schéma, une sortie TypeScript typée",
    intro: "Linguini analyse les contrats .lgs et la logique .lgl une fois, puis émet des modules ciblés avec formateurs CLDR, sélecteurs de pluriel et adaptateurs framework au build.",
    ts_title: "TypeScript / ESM",
    ts_desc: "Fonctions de messages typées, runtime partagé, fichiers de déclaration et tree shaking optionnel par locale.",
    svelte_title: "Svelte 5 + SvelteKit",
    svelte_desc: "Rune l générée, changement de locale, helpers localizeHref et exports handle / reroute / load.",
    planned_title: "Cibles prévues",
    planned_intro: "La feuille de route est volontairement large: Linguini vise tous les runtimes, langages et frameworks web pratiques.",
    rust: "Rust",
    kotlin: "Kotlin",
    swift: "Swift",
    go: "Go",
  ...
  hero: {
    eyebrow: "Localisation typée pour équipes produit",
    title: "Linguini",
    tagline: "L'internationalisation pour le reste d'entre nous",
    copy: "Un langage de localisation compilé où schémas, grammaire, formatage CLDR, hooks SvelteKit, cookies et routes localisées viennent d'une seule source.",
    term: "/lɪŋˈɡwiːni/",
    term_kind: "NOM",
    term_hint: "longs rubans de pâtes étroits",
    intro: "Linguini transforme des fichiers de locale liés au schéma en code généré typé. Avant la génération, check/build signalent les implémentations manquantes et valident les usages pris en charge des formatteurs, ainsi que les appels et branches de dispatch que l'analyseur peut résoudre.",
    trait_typed: "Typé",
    trait_compiled: "Compilé",
    trait_native: "Natif",
  ...
  playground: {
    ...
    title: "Change count, fruit, size, amount, date ou locale.",
    count_label: "Quantité",
    fruit_label: "Fruit",
    fruit_apple_label: "pomme",
    fruit_pear_label: "poire",
    fruit_orange_label: "orange",
    size_label: "Taille",
    size_small_label: "petit",
    size_big_label: "grand",
    amount_label: "Montant",
    date_label: "Date",
    localized_path_label: "Route localisée",
    cookie_label: "Cookie",
    route_label: "Préfixe route",
    sentence: (fruit: __lgl_name_6D61696E2E4672756974, size: __lgl_name_6D61696E2E53697A65, count: number | bigint | string, amount: __lgl_name_6D61696E2E4D6F6E6579, date: __lgl_name_6D61696E2E53686F727444617465) => String(__lgl_name_6D61696E2E44656C697665726564(count, __lgl_form_6D61696E2E4672756974[fruit].Gender)) + " " + String(formatNumber(count)) + " " + String(__lgl_name_6D61696E2E53697A6541646A(size, count, __lgl_form_6D61696E2E4672756974[fruit].Gender)) + " " + String(__lgl_form_6D61696E2E4672756974[fruit].nom(count)) + ". Total " + String(formatCurrency(amount, 2, 0, { code: "EUR" })) + "; date " + String(formatDate(date, { style: "short" })) + ".",
    cart_summary: (count: number | bigint | string, fruit: __lgl_name_6D61696E2E4672756974) => "Le panier contient " + String(formatNumber(count)) + " " + String(__lgl_form_6D61696E2E4672756974[fruit].nom(count)),
    number_format: (value: __lgl_name_6D61696E2E4D6561737572656D656E74) => "Format nombre: " + String(formatNumber(value)),
    currency_format: (amount: __lgl_name_6D61696E2E4D6F6E6579) => "Format devise: " + String(formatCurrency(amount, 2, 0, { code: "EUR" })),
    date_format: (date: __lgl_name_6D61696E2E53686F727444617465) => "Format date: " + String(formatDate(date, { style: "short" })),
  ...
  web: {
    kicker: "SvelteKit",
    title: "SvelteKit localisé sans câblage i18n manuel",
    ...
    fallback: "Les données de la locale de base sont intégrées aux modules générés pendant le build.",
    reactivity: "The l rune updates UI text immediately after setLocale.",
    links: "Certains liens peuvent garder leur URL d'origine quand la localisation n'est pas voulue.",
  },
} as const;
