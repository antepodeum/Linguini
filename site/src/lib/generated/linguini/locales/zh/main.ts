...
function pluralZh(value: number | bigint | string): string {
...
function formatDate(
  ...
  switch (options.style ?? "medium") {
    case "full":
      return String(date.getUTCFullYear()) + "年" + String(date.getUTCMonth() + 1) + "月" + String(date.getUTCDate()) + "日" + ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"][date.getUTCDay()];
    case "long":
      return String(date.getUTCFullYear()) + "年" + String(date.getUTCMonth() + 1) + "月" + String(date.getUTCDate()) + "日";
    case "short":
...
const __lgl_form_6D61696E2E4672756974 = {
  apple: { emoji: "🍎", nom: (value: number | bigint | string) => selectBranch(pluralZh(value), { _: "苹果" }) },
  pear: { emoji: "🍐", nom: (value: number | bigint | string) => selectBranch(pluralZh(value), { _: "梨" }) },
  orange: { emoji: "🍊", nom: (value: number | bigint | string) => selectBranch(pluralZh(value), { _: "橙子" }) },
} as const;

function __lgl_name_6D61696E2E53697A65576F7264(__lgl_p0: __lgl_name_6D61696E2E53697A65): string {
  return selectBranch(String(__lgl_p0), { small: (): string => "小", big: (): string => "大" })();
}

export const main = {
  codegen: {
    kicker: "代码生成",
    title: "一份 schema，生成类型化 TypeScript 输出",
  ...
  hero: {
    ...
    term_kind: "名词",
    term_hint: "细长扁平的意式面",
    intro: "Linguini 将与 schema 对应的 locale 文件转换为类型化生成代码。生成前，check/build 会报告缺失实现，并校验受支持的格式化器用法，以及分析器能够解析的 locale 调用和 dispatch 分支。",
    trait_typed: "类型化",
    trait_compiled: "编译型",
    trait_native: "原生",
    primary_cta: "阅读文档",
