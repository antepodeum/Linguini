...
fn browser_link_observer_is_owned_batched_and_bounded() {
    assert!(SVELTE_RUNTIME.contains("hot?.dispose(() => linguini.destroy())"));
    assert!(SVELTE_RUNTIME.contains("AUTO_LINK_MAX_PENDING_ROOTS"));
    assert!(SVELTE_RUNTIME.contains("AUTO_LINK_NODE_BUDGET"));
    assert!(SVELTE_RUNTIME.contains("shouldLocalizeLink"));
    assert!(!SVELTE_RUNTIME.contains("activeAutoLinkCleanup"));
    assert!(!SVELTE_RUNTIME.contains("querySelectorAll"));
}
