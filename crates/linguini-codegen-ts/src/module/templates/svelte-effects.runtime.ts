...
function startAutoLinkLocalization(getLocale: () => Locale) {
  ...
  const flush = () => {
    ...
    while (remaining > 0) {
      ...
      if (element.matches("a[href]")) {
        localizeAnchorElement(getLocale(), element);
      }
      remaining -= 1;
    }
  ...
  const onClick = (event: Event) => {
    ...
    if (anchor) localizeAnchorElement(getLocale(), anchor);
  };
...
function localizeAnchorElement(locale: Locale, anchor: Element) {
