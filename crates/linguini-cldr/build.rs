...
fn generate_plural_tables(source: &str) -> Result<String, String> {
    ...
    for (locale, value) in locales {
        ...
        for (key, value) in object {
            ...
            let rule = value
                .as_str()
                .ok_or_else(|| format!("plural rule `{key}` for locale `{locale}` is not a string"))?;
            categories.push((category.to_owned(), rule.to_owned()));
        }
        ...
        for (category, rule_source) in categories {
            let function = format!("plural_{}_{}", rust_identifier(locale), rust_identifier(&category));
